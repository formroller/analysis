# 데이터 분석
데이터를 통해 데이터의 숨겨진 의미있는 정보를 찾아가는 과정 => 미래 예측
...
# [ 데이터 분석의 분류 ]
1. 지도학습 : Y값 존재, 관심있는 Y값을 X로 설명/예측하는 작업
 1) 회귀 : Y가 연속형
 2) 분류 : Y가 범주형
2. 비지도학습 : Y값 존재X, 데이터로부터 유사성에 근거한 군집을 분류, 
               비슷한 특징을 갖는 데이터의 분류
...
- numpy, pandas : 정형, 비정형 데이터를 담기 위한 모듈
- scipy         : 복잡한 수학 연산
- matlotlib     : 그래프 출력
- mglearn       : 복잡한 시각화 함수 제공(팔레트 등), 
                  외부모듈로 아나콘다 미포함 => 설치 필요
            
run profile1
import mglearn    # pip install mglearn 이후 로딩 가능
...
df_iris['data']          # 설명변수의 집합
df_iris['target']        # 종속변수 데이터, 숫자형 범주데이터로 표현
...
x_train, x_test, y_train, y_test = train_test_split(x,  # 설명변수 데이터 셋
                                                    y,  # 종속변수 데이터 셋
                                                    train_size=0.75,  # train set 비중의 기본이 75%
...
x_train.shape   # (112, 4), trunc(150 * 0.75)의 row 추출
x_test.shape    # ( 38, 4)
...
# [ 참고 : score 메서드를 사용하지 않고 평가 점수 얻는 방법(R과 비슷) ] 
m_knn.predict(x_test)   # test set에 대한 예측 결과
y_test                  # test set에 대한 실제 정답
sum(y_test == m_knn.predict(x_test)) / y_test.shape[0] * 100

# 4. 매개변수 튜닝
# 4-1) 반복문을 통한 k값의 변화에 따른 train, test set의 score 계산
...
m_knn.predict(new_data)   # 에러발생, input 데이터가 2차원 형태가 아님
m_knn.predict(new_data2)  # 정상수행
...
# knn - 회귀
- knn은 기본은 분류모델이지만 회귀 모델도 제공
- 전통 회귀와는 다르게 인과관계 분석보단 예측에 초점을 맞춘 알고리즘
- 전통 회귀의 여러 통계적 가설(오차의 정규성, 등분산성, ...)을 따르지 않아도 됌

[ 참고 - R^2 ]
- knn-회귀 모델의 평가는 R^2로 제공
- R^2는 총 분산을 얼마나 회귀식으로 설명했는가를 의미

(y - ybar) = (y - yhat) + (yhat - ybar)
 
sum((y - ybar)^2) = sum((y - yhat)^2) + sum((yhat - ybar)^)
...
아래와 같은 설명변수를 갖는 경우 각 설명변수끼리의 상호작용 고려
x1 x2 x3

1) 2차항 고려 : x1^2, x2^2, x3^2, x1x2, x1x3, x2x3
1) 3차항 고려 : x1^2, x2^2, x3^2, x1x2, x1x3, x2x3,
               x1^3, x2^3, x3^3, x1^2x2, ...., x2x3^2, x1x2x3
               
               
               
# [ 연습 문제 ]
# cancer data set을 사용하여 knn 수행, 모델 평가, 매개변수 튜닝
